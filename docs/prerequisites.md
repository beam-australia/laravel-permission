...
In the `Installation` instructions you'll see that the `HasGroups` trait must be added to the User model to enable this package's features.
...
Additionally, your `User` model/object MUST NOT have a `group` or `groups` property (or field in the database), nor a `groups()` method on it. Those will interfere with the properties and methods added by the `HasGroups` trait provided by this package, thus causing unexpected outcomes when this package's methods are used to inspect groups and permissions.

Similarly, your `User` model/object MUST NOT have a `permission` or `permissions` property (or field in the database), nor a `permissions()` method on it. Those will interfere with the properties and methods added by the `HasPermissions` trait provided by this package (which is invoked via the `HasGroups` trait).
